...
'''
def Graph (option,k,k_cluster,k_centroid,pl_colorscale,color3,color2,d_cluster,color4,pl_colorscale2,titles,dim_option,dim_list):
    '''
    ...
    #data trends.
    if (dim_option == 2) or (dim_option == 3):                          #We run this check to see if the user has picked an actual option.
        x_option = dim_list[0] - 1            #Here we ask the user for an axis that will be plotted on the x dimension.
                                                                        #We ask for the letter identifier.
        y_option = dim_list[1] - 1            #Ask for y dimension
        
        if dim_option == 3:                                         #If the user had asked for 3 dimensions, then we ask for z.
            z_option = dim_list[2] - 1            #ask for Z dimension                        
    if (dim_option == 2) or (dim_option == 3):     #As long as dim_option isn't 3.
        trace = []                                                  #Make an empty list called trace.
        if dim_option == 2:                             #if the user picked 2 dimensions
