...
for c in range (0,k):                                                       #Run a for loop for one to the # of centroids (k)                                   
    ...
    while icheck == False:                                                  
        ...
        if (num in taken) == False:                                         #We check to see if this point has already been picked.
            centroid_list.append(dat2[num])                                 #If it hasn't been picked already, we append that point into the centroid list.
...
while move == True:                                         
    ...
    #There is something wrong occuring here. The data point that the I originally assigned a centroid to(chose as my starting centroid) is moving with the centroid.   
