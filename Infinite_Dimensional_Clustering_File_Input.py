#-----------------------------------------------Input------------------------------------------------------------------------------------------------------------------
filename = input("Please enter the filename of your data file.")    #The user inputs the filename.

Data = []
import csv
with open (filename) as txtfile:
    readtxtfile = csv.reader(txtfile, delimiter = ',')
    for row in readtxtfile: 
        Data.append(row) #Stores the data from the input file in list called Data.
...
def Density_Cluster(Data,iradius, Clusters):                        #This function classifies data points into clusters and noise points
    ...
    while (len(Data)>0):                                            #While Loop runs as long as the length of the list Data is more than 0.
        ...
        for icount in range (len(Data)-1,-1,-1):                    #For Loop runs in descending order for the number of data points 
            if Data[icount]=="Del":
                Data.remove(Data[icount])
        for C in Clusters:
            if len(C)==1:
                Noise.append(C)
                Clusters.remove(C)
...
print (Density_Cluster(Data, 600, Clusters))
